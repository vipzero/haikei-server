...
const loadWordCounts = async () => {
  const snap = await fdb
    ...
    .get()
  const res = {}
  snap.docs.forEach((v) => (res[v.id] = v.data()))
  return res
}

const setupCount = async (counts) => {
  const batch = fdb.batch()
  Object.entries(counts).forEach(([k, v]) => {
    batch.set(
      fdb.collection('hist').doc(EVENT_ID).collection('counts').doc(k),
      v
    )
  })
  await batch.commit()
...
const countupWords = async (words) => {
  ...
  words.forEach((word) => {
    batch.set(
      fdb.collection('hist').doc(EVENT_ID).collection('counts').doc(word),
