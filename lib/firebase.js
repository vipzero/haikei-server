...
const tmpFileName = 'data/tmp.png'
export const download = async (url) => {
  const response = await fetch(url, { method: 'GET' })
  await new Promise((resolve) =>
    response.body.pipe(fs.createWriteStream(tmpFileName)).on('finish', resolve)
  )
}
...
export const uploadByUrl = async (url, name) => {
  ...
  await bucket.upload(tmpFileName, {
    destination: path,
...
export const uploadByUrlAll = async (urls) => {
  ...
  for (const [i, url] of urls.entries()) {
    const { downloadUrl, path } = await uploadByUrl(url, `${timeId}_${i}.png`)
